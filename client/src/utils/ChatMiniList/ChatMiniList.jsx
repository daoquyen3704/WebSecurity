...
function ChatMiniList() {
  ...
  const handleOpenMessager = async (user) => {
    console.log(
      "Trying to open chat from mini list:",
    ...
    const existingUser = usersMessage.find(
      (existingUser) => String(existingUser.id) === String(user.sender.id)
    );
  ...
  return (
    <div className="chat-mini-list">
      {/* Ẩn nút Danh sách khi đang mở */}
      {!isOpen && (
        <div className="chat-mini-toggle" onClick={toggleOpen}>
          {unreadCount > 0 && <span className="chat-badge">{unreadCount}</span>}
          <span className="chat-label">Danh sách</span>
        </div>
      )}
                  ...
                  <div className="chat-mini-avatar">
                    ...
                    <span
                      className={`status-dot ${
                        user.sender.status === "Đang hoạt động"
                          ? "online"
                          : "offline"
                      }`}
                    ></span>
